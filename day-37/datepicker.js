const createOptionElement = (val, txt, appendTo = null) => {
  const option = document.createElement("option");
  option.value = val;
  option.text = txt;

  if (appendTo) document.querySelector(appendTo).appendChild(option);
  else return option;
};
...
class DatePicker {
  constructor(obj) {
    ...
    if (
      this.startDate.substr(0, 7) > this.defaultYearAndMonth ||
  ...
  setupEventListeners() {
    document
      ...
      .addEventListener("change", (event) => {
        this.currentYearAndMonth =
          event.target.value + this.currentYearAndMonth.substr(4, 3);
        this.setHeaderYearAndMonthRange();
      });
    ...
    document
      ...
      .addEventListener("change", (event) => {
        this.currentYearAndMonth =
          this.currentYearAndMonth.substr(0, 5) + event.target.value;
        this.renderDays();
      });
  ...
  setHeaderYearAndMonthRange() {
    ...
    document.querySelector(`#datepicker-${this.uid} .pick-month-select`).value =
      this.currentYearAndMonth.substr(5, 2);
  ...
  renderDays() {
    ...
    monthArray.forEach((day, index) => {
      if (index % 7 === 0) {
        const newRow =
          '<div class="datepicker-calendar--body__days-row"></div>';
      ...
      const calendarRows = document.querySelectorAll(
        `#datepicker-${this.uid} .datepicker-calendar--body__days-row`
      );
  ...
  highlightSelectedDay() {
    ...
    if (this.selectedDate.substr(0, 7) === this.currentYearAndMonth)
      document
        .querySelector(
          `#datepicker-${this.uid} .day-unit a[data-value='${this.selectedDate}']`
        )
  ...
  changeMonth(value) {
    ...
    if (
      this.startDate.substr(0, 7) <= newDate &&
